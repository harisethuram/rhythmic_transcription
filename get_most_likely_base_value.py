...
if __name__ == "__main__":
    ...
    base_path = "output/presentation_results/added_barlines_with_measure_lengths"
    for i, row in metadata.iterrows():
        piece = f"{row['piece_id']}_{row['piece_name']}_{row['part_id']}"
        results[piece] = []
        for base_value in base_values:  
            # this_path = os.path.join(base_path, piece + "_" + base_value + ".json")
            this_path = os.path.join(base_path, piece, f"base_value_{base_value}", "output.json")
    ...
    with open("basevalues_with_measure_lengths.json", "w") as f:
